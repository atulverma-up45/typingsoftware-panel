...
export function ProtectedRoute() {
  ...
  if (isLoading) {
    return (
      <div className="relative flex flex-col items-center justify-center min-h-screen w-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-orange-50/70 via-[#fcfcfc] to-[#f8fafc] px-4 overflow-hidden select-none">
        ...
        <div className="relative flex flex-col items-center max-w-md text-center z-10">
          ...
          <div className="relative mb-8 flex items-center justify-center">
            ...
            <div className="absolute w-44 h-44 sm:w-56 sm:h-56 md:w-60 md:h-60 rounded-full border border-dashed border-[#ff8a5c]/25 animate-spin-slow pointer-events-none" />

            {/* Middle Pulsing Halo Ring */}
            <div className="absolute w-36 h-36 sm:w-48 sm:h-48 md:w-52 md:h-52 rounded-full border border-[#ff8a5c]/30 animate-pulse-ring pointer-events-none" />

            {/* Soft Ambient Inner Glow */}
            <div className="absolute inset-2 rounded-full bg-gradient-to-tr from-[#ff8a5c]/25 via-orange-300/15 to-transparent blur-2xl animate-pulse pointer-events-none" />
          ...
          <div className="flex items-center justify-center text-3xl sm:text-4xl md:text-5xl font-black tracking-tight">
            <span className="bg-gradient-to-r from-[#ff8a5c] to-[#f77947] bg-clip-text text-transparent">
              Typing
            </span>
          ...
          <div className="flex items-center gap-2.5 mt-2 text-xs sm:text-sm font-semibold tracking-wider text-gray-400 uppercase">
            <span className="w-6 h-[2px] bg-[#ff8a5c]/50 rounded-full" />
          ...
          <div className="w-60 sm:w-72 md:w-80 h-2 bg-orange-100/60 rounded-full overflow-hidden mt-8 relative shadow-inner border border-orange-100/70">
            <div className="absolute inset-y-0 w-2/5 bg-gradient-to-r from-orange-400 via-[#ff8a5c] to-[#f77947] rounded-full animate-progress-shimmer shadow-[0_0_14px_rgba(255,138,92,0.7)]" />
          </div>
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/90 backdrop-blur-md border border-orange-100/80 shadow-xs mt-4">
            <span className="relative flex h-2 w-2">
              ...
              <span className="relative inline-flex rounded-full h-2 w-2 bg-[#ff8a5c]"></span>
            </span>
...
export function RoleRoute({ allowedRoles }: RoleRouteProps) {
  ...
  const isAllowed = hasAnyRole(userRole, allowedRoles);

  useEffect(() => {
    if (!isAllowed) {
      toast.error("Access Restricted", {
        description: `Your role (${userRole}) is not authorized to access this module.`,
      });
    }
  }, [isAllowed, userRole]);
