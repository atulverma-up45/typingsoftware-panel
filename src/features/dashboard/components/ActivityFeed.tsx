...
const ActivityFeed: React.FC = () => {
  ...
  return (
    <div className="bg-primary-tint rounded-[20px] shadow-sm border border-primary-tint-border p-6 flex flex-col h-[500px]">
      <div className="flex items-center gap-3 mb-6">
        <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-primary shadow-2xs border border-primary-tint-border">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        ...
        {isLoading ? (
          <div className="space-y-4">
            {Array.from({ length: 4 }).map((_, idx) => (
              <div key={idx} className="flex items-start gap-3 animate-pulse">
                ...
                <div className="flex-1 bg-white p-3 rounded-lg border border-primary-tint-border space-y-1.5">
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-6 text-gray-400">
            <div className="w-12 h-12 rounded-2xl bg-white text-primary flex items-center justify-center mb-3 shadow-2xs">
              <ShieldCheck size={24} />
            </div>
          ...
          <div className="space-y-5 relative before:absolute before:inset-0 before:ml-2.5 before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-primary/30 before:to-transparent">
            {events.map((event) => (
              <div key={event.id} className="relative flex items-start justify-between">
                <div className="flex items-start gap-4 z-10 w-full">
                  <div
                    className="w-5 h-5 rounded-full mt-0.5 shrink-0 border-4 border-primary-tint"
                    style={{ backgroundColor: event.color || '#f77f52' }}
                  />
                  <div className="flex-1 min-w-0 bg-white p-3 rounded-lg border border-primary-tint-border shadow-2xs hover:border-[#fc9b7f]/50 transition-colors">
