...
├── config/env.ts         # Zod-validated env — the only file that reads import.meta.env
...
├── lib/                  # api/client (axios), permissions, queryKeys
...
- Transport types come from `types/api.ts` (`PaginatedResponse<T>`, `ApiEnvelope<T>`); identity
  types from `types/auth.ts` (`UserRole`, `UserStatus`). No `any` in new code (`api.get<any, any>`
  is legacy — type it when you touch it).
...
`ConfirmDialog` (danger/primary variants). Do **not** hand-roll
...
- Errors surface via standardized messages from `lib/api/client.ts`; 401 clears the auth store.

## 10. Roadmap (deliberate, incremental)

1. Migrate remaining domains' query keys to `lib/queryKeys.ts` (users + dashboard done).
2. Decompress mega-pages (Activations ~1.6k lines) into `<Feature>Stats / Table / Card / Filters`
   co-located components + a `useListFilters` hook.
3. Convert feature modals to `<Modal>` and forms to RHF+Zod as each file is touched.
4. Type the legacy `api.get<any, any>` calls (institutions, activations, licenses, …).
5. Add Vitest unit tests for hooks/permissions and a Playwright smoke pass.